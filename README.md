...
```bash
claude          # Claude Code (預設讀 CLAUDE.md,本 repo 不含,啟動後請手動指定 AGENTS.md)
...
> 請依照 `AGENTS.md` 的規則，讀取 `drafts/article.md`，
...
3. 確保本機已設好 AWS CLI 認證（`aws configure`，憑證寫到 `~/.aws/credentials`）

> **AWS 憑證不要塞進本專案的 `.env`。**
> `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` 由 `boto3` 自動從 `~/.aws/credentials` 或環境變數讀取，本工具不會也不需要看到它們。建議為這個工具開一個 dedicated IAM user，policy 只開 `s3:PutObject` 到 `s3://<your-bucket>/threads-assets/*`，降低憑證一旦外洩的影響範圍。

> **S3 上傳的圖片會永遠公開可索引。**
> 上傳前請留意：
> - 圖片 URL 會被搜尋引擎索引，文章下架後圖還在
> - EXIF 中的 GPS、拍攝裝置資訊會跟著一起公開（建議上傳前用 `exiftool -all= <file>` 或 macOS 內建「預覽 → 工具 → 顯示檢閱器 → 刪除位置資訊」清掉）
> - 本工具目前沒有自動清理機制
...
```text
thread-weaver/
├── AGENTS.md             # AI 助理整理規則（給 Codex / Cursor / Claude Code / 任何 agent）
├── README.md             # 你正在看這個
...
- **不要把「未發表的草稿」貼到第三方 AI 服務。**
  把 `AGENTS.md` 規則檔貼到 ChatGPT / Claude.ai / Gemini 免費版沒問題（規則本來就公開），但若連同自己還沒發表的文章一起貼，那篇內容可能被該服務留 log 或拿去訓練。用網頁版 AI 處理草稿前，先確認該服務的資料使用條款，或改走 agentic CLI 路徑。
...
- **發文失敗時，先檢查錯誤訊息再貼出來求助。**
  `scripts/post_threads.py` 已經會在 raise 之前把 `THREADS_ACCESS_TOKEN` 從 Meta 的錯誤回應中替換成 `<redacted-access-token>`，但 Meta 偶爾會在錯誤裡帶其他敏感欄位（如 user id、debug trace）。把 traceback 貼到 issue / Slack / Discord 前，再人眼掃一次。
